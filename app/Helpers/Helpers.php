...
// function env_api(){
    ...
    // $data['dev1']['token'] = 'token 3a425aa71a6f09e:f865b0faf191cfc';
...
function getItemPO_v2(array $_filters){
    // pr($_filters);
    $doctype            = 'Purchase Receipt Schedule';
    $start 			    = 0;
    $page_length 	    = 999;
    $order_by           = 'name desc';
    $fields 		    = "purchase_order,item_code,stock_uom,qty,rate";

    $filters['schedule_date']      = ['IN',implode(',',$_filters['schedule_date'])];
    ...
    if(count($datas)){
        foreach($datas as $k => $val){
            $data_items[$val->schedule_date][$val->item_code] = $val;
        }
    }
    return $data_items;
}
